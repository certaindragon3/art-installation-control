...
export function ClassroomMap({
  ...
  return (
    <div className={cn("flex flex-col gap-3", className)}>
      <div className="grid grid-cols-[minmax(0,1fr)_auto] grid-rows-[auto_minmax(0,1fr)] gap-x-3 gap-y-2">
        <div className="col-start-1 row-start-1 flex items-center justify-between px-6 text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
          <span>Left</span>
          <span>Right</span>
        </div>

        <div className="col-start-1 row-start-2 relative aspect-[4/3] overflow-hidden rounded-[1.5rem] border border-border/60 bg-muted/20">
          <div
        ...
        <div className="col-start-2 row-start-2 flex h-full flex-col items-center justify-between py-6 text-[10px] uppercase tracking-[0.18em] text-muted-foreground">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-border/60 bg-background/70 px-3 py-2 text-xs text-muted-foreground">
        <span>Left → Right {xPercent.toFixed(1)}</span>
        <span>Back → Front {yBackToFrontPercent.toFixed(1)}</span>
      </div>
    </div>
  );
}
