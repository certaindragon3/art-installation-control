...
- The only issue found in the first smoke was `POST /api/unity/register` exporting `socketServerUrl` with `http://` instead of `https://` behind the reverse proxy.
- That issue was fixed in [server/_core/app.ts](/Users/huangjiesen/大四/s4/G哥项目/art-installation-control/server/_core/app.ts:90), covered in [server/controllerApi.test.ts](/Users/huangjiesen/大四/s4/G哥项目/art-installation-control/server/controllerApi.test.ts:2062), redeployed, and rechecked live on 2026-04-22.

## Deployment / Runtime Notes

- A live Zeabur smoke was run after deployment at `https://artinstallation.certaindragon3.work`.
- The first production smoke found one reverse-proxy issue in the Unity registration metadata path: exported `socketServerUrl` used `http://` instead of the public `https://` origin.
- The root cause was missing Express proxy trust. The code now sets `app.set("trust proxy", true)`, and `/api/unity/register` has coverage for `x-forwarded-proto: https`.
- After redeploy, live recheck confirmed `POST /api/unity/register` now returns the correct public `https://` origin.
